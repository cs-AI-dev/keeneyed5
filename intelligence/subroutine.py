...
def srt_02(): # Reset short-term memory caches
	...
	for x in range(10):
		...
		with open(__file__[:-13] + "memory/longterm/memory_cache_" + str(x) + ".ke5", "r") as f:
			cli.display(f.read())
		f = open(__file__[:-13] + "memory/longterm/memory_cache_" + str(x) + ".ke5", "w")
...
def srt_03(day=datetime.datetime.today().day, month=datetime.datetime.today().month, year=datetime.datetime.today().year): # Generate new long-term memory cache
	...
	with open(__file__[:-3] + "/longterm/" + "d" + str(day) + "m" + str(month) + "y" + str(year) + ".ke5", "x") as f:
		pass
	with open(__file__[:-3] + "/longterm/" + "d" + str(day) + "m" + str(month) + "y" + str(year) + ".ke5", "a") as f:
